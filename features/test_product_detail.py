from screenpy import Actor, given, then, when
from screenpy.actions import See
from screenpy.pacing import act, scene
from screenpy_selenium.actions import Open
from screenpy.resolutions import (
    ContainsTheText,
    IsEqualTo
)
from screens.login import LoginPage
from screens.home_screen import HomePage
from tasks.login_task import LogIn
from tasks.add_product_task import AddProductTask
from questions.SearchNumberCart import SearchNumberCart
from questions.SearchLogInResult import SearchLogInResult
from tasks.open_product_detail_task import OpenProductDetailTask
from questions.SearchText import SearchText
from questions.SearchValue import SearchValue

@act("Search")
@scene("open producct detail")
def test_add_product(Dev: Actor) -> None:
    given(Dev).was_able_to(Open.their_browser_on(LoginPage().URL), LogIn.log_in(), Open.their_browser_on(HomePage.URL))
    name_product = SearchText(HomePage.CHECK_NAME)
    when(Dev).attempts_to(OpenProductDetailTask())
    then(Dev).should(
        See.the(SearchText(HomePage.CHECK_DETAIL), IsEqualTo('Sauce Labs Backpack')),
    )
